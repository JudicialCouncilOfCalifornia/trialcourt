...
namespace Drush\Commands\jcc_base;

use Consolidation\AnnotatedCommand\CommandData;
use Consolidation\AnnotatedCommand\CommandError;
use Drupal\Component\Utility\UrlHelper;
use Drupal\jcc_base\SubThemeGenerator;
...
class SubThemeCommands extends DrushCommands implements BuilderAwareInterface {
  ...
  /**
   * Creates a JCCBase sub-theme.
   *
   * @command jcc_base:create
   * @aliases jcc_base
   *
   * @bootstrap full
   *
  ...
  public function generateSubTheme(
    ...
    if (UrlHelper::isValid($kit, TRUE)) {
      ...
      $cb->addCode(function (RoboStateData $data) use ($kitUrl): int {
        $logger = $this->logger();
        $logger->debug(
          'download JCCBase starter kit from <info>{kitUrl}</info>',
          [
            'kitUrl' => $kitUrl,
          ]
        );
      ...
      $cb->addCode(function (RoboStateData $data): int {
        $logger = $this->logger();
        $logger->debug(
          'extract downloaded JCCBase starter kit from <info>{packPath}</info> to <info>{srcDir}</info>',
          [
    ...
    $cb->addCode(function (RoboStateData $data) use ($dstDir): int {
      $logger = $this->logger();
      $logger->debug(
        'copy JCCBase starter kit from <info>{srcDir}</info> to <info>{dstDir}</info>',
        [
          'srcDir' => $data['srcDir'],
          'dstDir' => $dstDir,
        ]
    ...
    $cb->addCode(function () use ($name, $options, $dstDir): int {
      $logger = $this->logger();
      $logger->debug(
        'customize JCCBase starter kit in <info>{dstDir}</info> directory',
        [
          'dstDir' => $dstDir,
        ]
      );
