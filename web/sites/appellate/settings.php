 ...
 * @endcode
 */
$settings['hash_salt'] = 'sUWTlxul-QzeZniVkHiogIwmOIaZ6KM1KVFZZMuMZm69ERV_MoUuXG2DDObejtzwpGuOW6akNQ';
...
$settings['config_sync_directory'] = '../config/config-appellate';
$config['config_split.config_split.local']['folder'] = '../config/config-appellate-local';
$settings['file_public_path']  = 'sites/default/files/appellate/default';
...
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
  ...
  if (in_array($_ENV['PANTHEON_ENVIRONMENT'], array('live', 'dev'))) {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-appellate-live';
  }
  else if ($_ENV['PANTHEON_ENVIRONMENT'] == 'stage') {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-appellate-stage';
  }
  else {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-appellate-develop';
  }
}
...
// Assumes local.
  ...
  $config['config_split.config_split.stage']['status'] = TRUE;
  $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-appellate-sandbox-1';
}
