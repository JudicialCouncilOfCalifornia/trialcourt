...
$settings['config_sync_directory'] = '../config/config-cjeo';
$config['config_split.config_split.local']['folder'] = '../config/config-cjeo-local';
$settings['file_public_path']  = 'sites/default/files/cjeo/default';
...
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
  ...
  if (in_array($_ENV['PANTHEON_ENVIRONMENT'], array('live', 'dev'))) {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-cjeo-live';
  }
  else if ($_ENV['PANTHEON_ENVIRONMENT'] == 'stage') {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-cjeo-stage';
  }
  else {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-cjeo-develop';
  }
}
...
// Assumes local.
  ...
  $config['config_split.config_split.stage']['status'] = TRUE;
  $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-cjeo-sandbox-1';
}
