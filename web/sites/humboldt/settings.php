...
$settings['config_sync_directory'] = '../config/config-humboldt';
$config['config_split.config_split.local']['folder'] = '../config/config-humboldt-local';
$settings['file_public_path']  = 'sites/default/files/humboldt/default';
...
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
  ...
  if (in_array($_ENV['PANTHEON_ENVIRONMENT'], array('live', 'dev'))) {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-humboldt-live';
  }
  else if ($_ENV['PANTHEON_ENVIRONMENT'] == 'stage') {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-humboldt-stage';
  }
  else {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-humboldt-develop';
  }
}
...
// Assumes local.
  ...
  $config['config_split.config_split.stage']['status'] = TRUE;
  $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-humboldt-sandbox-1';
}
