...
$settings['config_sync_directory'] = '../config/config-md';
$config['config_split.config_split.local']['folder'] = '../config/config-md-local';
$settings['file_public_path']  = 'sites/default/files/md/default';
...
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
  ...
  if (in_array($_ENV['PANTHEON_ENVIRONMENT'], array('live', 'dev'))) {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-md-live';
  }
  else if ($_ENV['PANTHEON_ENVIRONMENT'] == 'stage') {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-md-stage';
  }
  else {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-md-develop';
  }
}
...
// Assumes local.
  ...
  $config['config_split.config_split.stage']['status'] = TRUE;
  $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-md-sandbox-1';
}

/**
 * Enable redis in Pantheon.
 */
