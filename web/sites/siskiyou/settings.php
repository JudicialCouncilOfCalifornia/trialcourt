...
$settings['config_sync_directory'] = '../config/config-siskiyou';
$config['config_split.config_split.local']['folder'] = '../config/config-siskiyou-local';
$settings['file_public_path']  = 'sites/default/files/siskiyou/default';
...
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
  ...
  if (in_array($_ENV['PANTHEON_ENVIRONMENT'], array('live', 'dev'))) {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-siskiyou-live';
  }
  else if ($_ENV['PANTHEON_ENVIRONMENT'] == 'stage') {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-siskiyou-stage';
  }
  else {
    $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-siskiyou-develop';
  }
}
...
// Assumes local.
  ...
  $config['config_split.config_split.stage']['status'] = TRUE;
  $config['search_api.server.solr']['backend_config']['connector_config']['core'] = 'jcc-siskiyou-sandbox-1';
}
