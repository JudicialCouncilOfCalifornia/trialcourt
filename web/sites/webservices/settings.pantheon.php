 ...
 *
 * Note that this location is protected when running on the Pantheon
 * environment, but may be exposed if you migrate your site to
 * another environment.
 */
$settings['file_private_path'] = 'sites/default/files/webservices/default/private';

// Check to see if we are serving an installer page.
$is_installer_url = (strpos($_SERVER['SCRIPT_NAME'], '/core/install.php') === 0);

/**
 * Add the Drupal 8 CMI Directory Information directly in settings.php.
 *
...
if ($is_installer_url) {
  $config_directories = [
    CONFIG_SYNC_DIRECTORY => 'sites/default/files/webservices',
  ];
}
else {
  $config_directories = [
    CONFIG_SYNC_DIRECTORY => '../config/config-webservices',
  ];
}
...
/**
 * Override $databases variable.
 *
 * To pass the correct Database credentials directly from Pantheon to Drupal.
 *
...
if (isset($_ENV['PANTHEON_ROLLING_TMP']) && isset($_ENV['PANTHEON_DEPLOYMENT_IDENTIFIER'])) {
  ...
  // deployment identifier changes.  Note that cache rebuild is also necessary.
...
/**
 * Install Pantheon Service Provider to hook Pantheon services into Drupal 8.
 *
 * This service provider handles operations such as clearing the
...
/**
 * Trusted host settings are not necessary on Pantheon.
 *
 * Traffic will only be routed to your site if the host settings match a
 * domain configured for your site in the dashboard.
 */
if (isset($_ENV['PANTHEON_ENVIRONMENT'])) {
