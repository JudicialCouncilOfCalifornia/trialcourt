 ...
 *
 * @FieldWidget(
 *   id = "link_email_with_attributes",
 *   label = @Translation("Link (with email and attributes)"),
 *   field_types = {
 *     "link"
 *   }
 * )
 */
class LinkEmailWithAttributesWidget extends LinkWithAttributesWidget {
   ...
   *
   * @return string
   *   The URI as a displayable string.
   *
  ...
  protected static function getUriAsDisplayableString($uri): string {
    ...
    if ($scheme === 'mailto:') {
      $displayable_string = ltrim($displayable_string);
    }
  ...
  protected static function getUserEnteredStringAsUri($string): string {
    ...
    elseif (str_starts_with($string, 'mailto:')) {
      $uri = $string;
    }
  ...
  public static function validateUriElement($element, FormStateInterface $form_state, $form): void {
    ...
    if (parse_url($uri, PHP_URL_SCHEME) === 'internal'
      ...
      && !str_starts_with($element['#value'], 'mailto')) {
   ...
   */
  public static function validateTitleElement(&$element, FormStateInterface $form_state, $form): void {
   ...
   */
  public static function validateTitleNoLink(&$element, FormStateInterface $form_state, $form): void {
    if ($element['uri']['#value'] === '' && $element['title']['#value'] !== '') {
      $message = new TranslatableMarkup('The @uri field is required when the @title field is specified.', [
        '@title' => $element['title']['#title'],
        '@uri' => $element['uri']['#title'],
      ]);
      $form_state->setError($element['uri'], $message);
    }
  }
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state): array {
    ...
    if (!$this->supportsExternalLinks()) {
      ...
      $description = $this->t('This must be an internal path such as %add-node. You can also start typing the title of a piece of content to select it. Enter %front to link to the front page. Enter %nolink to display link text only. Enter %button to display keyboard-accessible link text only.', [
        '%add-node' => '/node/add',
        '%front' => '<front>',
        '%nolink' => '<nolink>',
        '%button' => '<button>',
      ]);
      $element['uri']['#description'] = $description;
    }

    // If the field is configured to allow both internal and external links,
    // show a useful description.
    elseif ($this->supportsExternalLinks() && $this->supportsInternalLinks()) {
      $description = $this->t('Start typing the title of a piece of content to select it. You can also enter an internal path such as %add-node or an external URL such as %url. Enter %front to link to the front page. Enter %nolink to display link text only. Enter %button to display keyboard-accessible link text only. For an email link, make sure to start with %mailto: such as %mailto_example', [
        '%front' => '<front>',
        '%add-node' => '/node/add',
        ...
        '%button' => '<button>',
        '%mailto:' => 'mailto:',
        '%mailto_example' => 'mailto:text@example.com',
      ]);
      $element['uri']['#description'] = $description;
    }
    ...
    $element['options']['attributes']['#open'] = FALSE;
