...
class JccRocRuleUniqueIdConstraintValidator extends ConstraintValidator implements ContainerInjectionInterface {
  ...
  public function validate($items, Constraint $constraint): void {
    ...
    if ($bundle == 'rule_subsection') {
      $unique_field_name = 'field_rule_subsection_id';
      if ($this->uniqueValidation($unique_field_name, $value, $entity_type, $bundle_field, $bundle, $id_field)) {
        $this->context->addViolation($constraint->nonUniqueRuleSubsectionId, [
          '%value' => $value,
        ]);
      }
    }
    ...
    if ($bundle == 'rule_document') {
      $unique_field_name = 'field_rule_document_id';
      if ($this->uniqueValidation($unique_field_name, $value, $entity_type, $bundle_field, $bundle, $id_field)) {
        $this->context->addViolation($constraint->nonUniqueRuleDocumentId, [
          '%value' => $value,
        ]);
      }
    }
