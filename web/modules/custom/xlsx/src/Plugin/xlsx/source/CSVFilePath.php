...
class CSVFilePath extends XlsxSourceBase {
  ...
  public function sourceForm(array $form, FormStateInterface $form_state) {
    $form['csv'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Absolute path to a CSV file'),
      '#description' => $this->t('Absolute path to a CSV file'),
      '#required' => TRUE,
    ];
    $form['cron'] = [
      '#type' => 'checkbox',
  ...
  public function submitSourceForm(array &$form, FormStateInterface $form_state) {
    $csv_file = $form_state->getValue('csv', '');
    if (!empty($csv_file)) {
      $params = [
  ...
  public function submitImportForm(array &$form, FormStateInterface $form_state, $xlsx = NULL) {
    ...
    if ($source_type == 'path') {
      $file_path = $form_state->getValue('csv', '');
      if (!empty($file_path)) {
