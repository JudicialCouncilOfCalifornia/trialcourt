...
class Spreadsheets
{
    ...
    public static function newSpreadsheet($phpSpreadsheet=NULL)
    {
        ...
        elseif (is_string($phpSpreadsheet)) {
            ...
            self::$_objSpreadsheet = IOFactory::load($phpSpreadsheet);
        }
        else {
    ...
    /** 
     * Output file to browser or upload to remote.
     */
    public static function process($xlsx_entity, $xlsx_export, $xlsx_remote, $password = '')
    {
        $filename = $xlsx_entity->label();
        $format = $xlsx_export->getDataType();
        ...
        if ($format == 'Pdf' && class_exists('\Mpdf\Mpdf')) {
            ...
            $objWriter = IOFactory::createWriter($objPhpSpreadsheet, 'Pdf');
        }
        else {
            // Create Writer first
            $objWriter = IOFactory::createWriter($objPhpSpreadsheet, $format);
            if ($format == 'Csv') {
                if (!empty($mapping['csv'])) {
                    $params = $mapping['csv'];
                    $objWriter->setDelimiter($params['delimiter']);
                    $objWriter->setEnclosure($params['enclosure']);
                }
            }
        }
        ...
        // If password protection required, lets add it into a zip file.
        // PHP spreadsheet library does not support password protection.
        if (!empty($password) && class_exists('\ZipArchive')) {
            ...
            $file = \Drupal::service('file_system')->tempnam($tmp_dir, 'zip');
            $zip = new \ZipArchive();
            if ($res = $zip->open($file, \ZipArchive::OVERWRITE)) {
              $zip->addFromString($filename . '.' . $extension, $file_contents);
              $zip->setEncryptionName($filename . '.' . $extension, \ZipArchive::EM_AES_256, $password);
              $zip->close();
            }
            ob_start();
            readfile($file);
            ...
            ob_end_clean();
            $xlsx_remote->process($contents, $filename, filesize($file), '.zip', 'application/zip');
            unlink($file);
        }
        else {
            ob_start();
            ...
            $file_contents = ob_get_contents();
            ob_end_clean();
            $xlsx_remote->process($file_contents, $filename, 0, $extension, $content_type);
        }
    }
    ...
    public static function getRow($toString=true, $options=[], callable $callback=null)
    {
        ...
        for ($col = $startColumn + 1; $col <= $columns; ++$col) {
            ...
            $value = $cell->getValue();
            // Timestamp option
            if ($options['timestamp'] && Date::isDateTime($cell)) {
