...
class JccRocRuleService {
   ...
   */
  public function getRuleFromId($rule_id): bool|EntityInterface {
    $node_manager = $this->entityTypeManager->getStorage('node');
    $subsection = $node_manager->load($rule_id);
    return $subsection && $subsection->bundle() == 'roc_rule' ? $subsection : FALSE;
  }

  /**
   * Get Rule documents that are Published and Promoted to ROC index.
   */
  public function getRocPublishedPromotedRuleIndexes(): array {
    $node_manager = $this->entityTypeManager->getStorage('node');
   ...
   */
  public function getRocAllRuleIndexes(): array {
    $node_manager = $this->entityTypeManager->getStorage('node');
    $query = $node_manager->getQuery();
    $nids = $query->condition('type', 'roc_rule_index')
      ->accessCheck(FALSE)
   ...
   */
  public function getRocDocumentAttachedMediaFile($subsection): bool|EntityInterface {
    if ($attached_media = $subsection->get('field_roc_rule_media')->referencedEntities()) {
   ...
   * Get the direct path to the (first) media item attached to a rule document.
   */
  public function getRocDocumentAttachedMediaFilePath($subsection): bool|string {
    if ($attached_media = $subsection->get('field_roc_rule_media')->referencedEntities()) {
