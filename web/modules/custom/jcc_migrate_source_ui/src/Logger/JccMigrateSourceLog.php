...
namespace Drupal\jcc_migrate_source_ui\Logger;
...
/**
 * Logs events in the jcc_migrate_source_log database table.
 */
class JccMigrateSourceLog implements LoggerInterface {
  use RfcLoggerTrait;
   ...
   */
  const DEDICATED_JMSL_CONNECTION_TARGET = 'dedicated_jcc_migrate_source_ui';
  ...
  /**
   * Constructs a JccMigrateSourceLog object.
   *
  ...
  public function log($level, $message, array $context = []) {
    ...
    try {
      ...
      if (
        ...
        $this->connection->getTarget() != self::DEDICATED_JMSL_CONNECTION_TARGET
      ) {
        // Open a dedicated connection for logging.
        $key = $this->connection->getKey();
        $info = Database::getConnectionInfo($key);
        Database::addConnectionInfo($key, self::DEDICATED_JMSL_CONNECTION_TARGET, $info['default']);
        $this->connection = Database::getConnection(self::DEDICATED_JMSL_CONNECTION_TARGET, $key);
